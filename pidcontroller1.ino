...
void loop() {
  ...
  if (currentTemp < TARGET_TEMP) {
    // HEATING mode
    digitalWrite(directionPin, HIGH);   // Assuming HIGH is heating
    analogWrite(pwmPin, output);       // Set PWM signal for heating
  } else {
    // COOLING mode
    digitalWrite(directionPin, HIGH);   // Assuming LOW is cooling
    analogWrite(pwmPin, output);       // Set PWM signal for cooling
  }
